...
tools/check-lib.js     sanity checks on makeDefaultLib() — node, no browser (npm run check)
...
```
`npm run voice`, `npm run sfx` (both read `.env`), `npm run check`, `npm run build`, `npm run serve`.
...
- **`fx` per keyframe**: a name the player hands to the game (`makePlayer(doll, {fx})`), which draws an overlay: `ball` (flies in and bonks the head), `headstars`, `sparks` (from both torch tips), `flame` (jet out of the bum), `discoball`, `confetti` (40 falling flakes), `zzz` (one floating Z per call), `scoopdrop` (hides the `scoop` prop, drops a blob onto the right shoe, splat stays until the next round), `bird` (fly-by with caw, no poop; `flyBird()` is shared with `birdPoop()`). Handlers live in `FX` in game.html and use `jointXY(joint, dx, dy)` for screen position + rotation of a joint.
- **Props** (`PROPS` in rig.js): costume pieces attached to a joint, hidden unless `anim.props` lists them: torchL/torchR, afro, shades, flareL/flareR, discoShirt, flagL/flagR (pennants on long sticks), partyHat (cone that covers the green hat), cone/scoop (ice cream on the right hand), bigHandL/bigHandR (r 14 palms that cover the eyes). `z:'back'|'mid'` controls layering; `doll.setProps(list)`. `anim.music` names a long mp3 the game starts/stops around the animation.
- **`anim.front`**: arms are normally drawn behind the head (rig ORDER). `front:true` makes the player call `doll.setFront(true)`, which moves lUpper/rUpper after `head` in the torso group for the duration (hands over the eyes, ice cream at the mouth). Hand positions were solved with a small forward-kinematics search; hands on the eyes = upper ±115°, lower ±85°.
- Animation naming drives game behaviour: `idle_*` (between rounds), `slap_*` (correct answer, 68%), `cheer_*` (12%), `bonus_poop` (10%, bird overlay), `bonus_animal` (10%, looped while a cow/pig/sheep peeks in or walks by and leaves a poo — `ANIMALS` in game.html), `mild_*` (wrong answer), `tap_belly`/`tap_head` (kid pokes the character; head incl. hat = stars, anything else = toot), `dance_*` (finale when the 10 bunting flags are full, 10 s, with `music`; riverdance, disco, party), `speak_point`, `count_hop`. Add a `slap_xyz` and the game picks it up.
- Non-fart `slap_*` gags (2026-09-11): `slap_peekaboo` (hands over eyes, peek, "Kiekeboe!", second reveal with the hat jumping), `slap_icecream` (two licks, scoop drops on his shoe, "Oh nee", eats the cone anyway), `slap_bounce` (three hops each higher, hat hangs in the air and lands on his head), `slap_sleep` (yawn, hat over the eyes, two snores with Zs, bird caw wakes him with a jump). The party gag (flags, party hat, horn, confetti, hops) became `dance_party` (10 s: horns, hops, headspins, horn salvo, jump with flying hat) and left the `slap_*` pool; game.html deletes a stale `lib.slap_party` from an older animations.json.
- Sounds: `SFX.play(name)` returns the Audio element — names: fart, bigfart, boing, thud, slip, whistle, splat, sneeze, ding, pop, spin, plof (dull body hit), oef, au (short yelps in the game voice via TTS), bird (kaaa), bonk, moo, oink, baa, snicker (Terrance giggling, eleven_v3 audio tags like [giggles]/[laughs] in tools/build-sfx.js YELPS), sparkle (3 s fizz), whoosh, disco/jig/party (10 s music, no synth fallback; a music name must also be in the synth table `S` or `SFX.play` ignores it), horn (party blower), slurp, snore (2 variants), yawn/kiekeboe/mmm/ohnee (TTS yelps with v3 tags like [yawning], [sad]). Comedy rule from Thomas: short, fast sounds in quick succession. Lookup order: baked `window.ASSETS.sounds` → `sounds/<name>.mp3` next to the html → WebAudio synth. Variants: `fart_1.mp3`, `fart_2.mp3` … are picked at random for `fart` (any name works the same).
...
Start overlay ("Spelen!") unlocks audio. Rounds cycle `letter, letter, number, letter, word`. Letters come from the configured names (so Mama/Papa/kids' names first), distractor from the alphabet. Numbers 1–5 show dots too and are counted with hops. Words are the names. Correct → stars + slapstick + praise; wrong → mild anim + "Hmm, nee." + wrong bubble shakes, correct bubble pulses, target repeated. Idle animations fire randomly while waiting; tapping the character while waiting plays `tap_head`/`tap_belly` (bubble taps still win). Bunting ("slinger"): under the cloud hangs a rope with 10 flags (`drawSlinger`/`paintFlags`/`addFlag`/`waveFlags`/`dropFlags` in game.html; count in `cfg.flags`, localStorage, survives settings saves and app restarts). Every correct answer a star flies from Terrance to the next flag, which takes its colour with a pop. At 10: `danceParty()` = flags wave with the horn → random `dance_*` with its music → praise → flags drop off one by one with pops → count back to 0. The `score` counter is invisible on purpose and no longer drives the dance. The svg has `overflow:visible` and oversized sky/earth rects so tall phones never show the body background. Settings (⚙): names, language, optional ElevenLabs key/voice, absurdity slider, paste animations.json. Persisted in localStorage.
...
4b. DONE (2026-09-11): five non-fart gags (party, peekaboo, ice cream, bounce, sleep), see the `slap_*` list above. Verified with Playwright (poses, props, fx, no JS errors) but not yet on the phone; the peekaboo forearms cross the face a bit and the ice-cream elbow bends inward — tune in the studio if it bothers. Rejected ideas kept for later: bellenblazen, ballon, toveren met konijn.
5. Ideas not built yet: photo heads (the `top` and `head` groups are clipped halves designed so a photo can be dropped into each), a Phillip character, more `slap_*` gags, letters of the kids' own names as a level, sound preview button in the studio. Plans/design docs go in `plans/`, never `docs/` (that is the Pages output).
6. DONE (2026-09-11, plan in `plans/2026-09-11-slinger-finale.md`, design in `plans/2026-09-11-slinger-finale-design.md`): bunting towards the party, `dance_party` 10 s with `party.mp3`, `npm run check`. Verified with Playwright; not yet on the phone: bunting height vs. the hat during jumps, horn/praise overlap, whether the generated party music is any good (else drop an own 10 s `party.mp3` in `assets/sounds/` and rebuild).
