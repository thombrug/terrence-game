...
- Non-fart `slap_*` gags (2026-09-11): `slap_party` (flags, party hat, horn, confetti, hops), `slap_peekaboo` (hands over eyes, peek, "Kiekeboe!", second reveal with the hat jumping), `slap_icecream` (two licks, scoop drops on his shoe, "Oh nee", eats the cone anyway), `slap_bounce` (three hops each higher, hat hangs in the air and lands on his head), `slap_sleep` (yawn, hat over the eyes, two snores with Zs, bird caw wakes him with a jump).
- Sounds: `SFX.play(name)` returns the Audio element — names: fart, bigfart, boing, thud, slip, whistle, splat, sneeze, ding, pop, spin, plof (dull body hit), oef, au (short yelps in the game voice via TTS), bird (kaaa), bonk, moo, oink, baa, snicker (Terrance giggling, eleven_v3 audio tags like [giggles]/[laughs] in tools/build-sfx.js YELPS), sparkle (3 s fizz), whoosh, disco/jig (10 s music, no synth fallback), horn (party blower), slurp, snore (2 variants), yawn/kiekeboe/mmm/ohnee (TTS yelps with v3 tags like [yawning], [sad]). Comedy rule from Thomas: short, fast sounds in quick succession. Lookup order: baked `window.ASSETS.sounds` → `sounds/<name>.mp3` next to the html → WebAudio synth. Variants: `fart_1.mp3`, `fart_2.mp3` … are picked at random for `fart` (any name works the same).
