...
function pose(over={}){const p=defaultPose();for(const k in over){if(!(k in p))continue;if(typeof over[k]==='object')Object.assign(p[k],over[k]);else p[k]=over[k];}return p;}
...
const SHAPES={
  ...
  top(g){const id='clipTop'+(clipN++);const cp=el('clipPath',{id});cp.append(el('rect',{x:-60,y:-90,width:120,height:74}));g.append(cp);
    ...
    g.append(t);
    for(const s of[-1,1])g.append(el('ellipse',{class:'eyewhite',cx:s*12,cy:-30,rx:9,ry:11,fill:'#fff',...OUT}));},
...
// z: 'back' = behind the joint's own shape, 'mid' = over the shape but under child limbs, default = on top of everything
const torch=g=>{g.append(el('rect',{x:-4,y:-34,width:8,height:40,rx:3,fill:'#8a5a2b',...OUT}));g.append(el('rect',{x:-6,y:-38,width:12,height:8,rx:2,fill:'#c8322b',...OUT}));};
const flare=g=>g.append(el('path',{d:'M-8,0 L-22,26 L22,26 L8,0 Z',fill:'#8e44ad',...OUT}));
const PROPS={
  torchL:{joint:'lHand',draw:torch}, torchR:{joint:'rHand',draw:torch},
  afro:{joint:'head',z:'back',draw:g=>{for(const [x,y,r] of [[0,-56,40],[-34,-40,26],[34,-40,26],[-40,-14,20],[40,-14,20],[-18,-70,22],[18,-70,22]])g.append(el('circle',{cx:x,cy:y,r,fill:'#2b2622',...OUT}));}},
  ...
  flareL:{joint:'lShin',z:'back',draw:flare}, flareR:{joint:'rShin',z:'back',draw:flare},
  discoShirt:{joint:'torso',z:'mid',draw:g=>g.append(el('rect',{x:-32,y:-48,width:64,height:60,rx:14,fill:'#f39c12',...OUT}))},
  // party: a pennant on a stick in each hand (pointing away from the head) and a cone hat that covers the green one
  flagL:{joint:'lHand',draw:g=>{g.append(el('rect',{x:-3,y:-70,width:6,height:74,rx:3,fill:'#8a5a2b',...OUT}));g.append(el('path',{d:'M-3,-70 L-38,-58 L-3,-46 Z',fill:'#c8322b',...OUT}));}},
  flagR:{joint:'rHand',draw:g=>{g.append(el('rect',{x:-3,y:-70,width:6,height:74,rx:3,fill:'#8a5a2b',...OUT}));g.append(el('path',{d:'M3,-70 L38,-58 L3,-46 Z',fill:'#3b8bff',...OUT}));}},
  partyHat:{joint:'hat',draw:g=>{g.append(el('path',{d:'M-36,14 L36,14 L0,-60 Z',fill:'#3b8bff',...OUT}));
    for(const [x,y] of [[0,-30],[-10,-6],[12,-4],[-22,8],[24,8]])g.append(el('circle',{cx:x,cy:y,r:4,fill:'#ffd400'}));
    g.append(el('circle',{cx:0,cy:-60,r:8,fill:'#c8322b',...OUT}));}},
  // bigger hands so they really cover the eyes in the peekaboo
  bigHandL:{joint:'lHand',draw:g=>g.append(el('circle',{cx:0,cy:6,r:14,fill:'#f2c9a0',...OUT}))}, bigHandR:{joint:'rHand',draw:g=>g.append(el('circle',{cx:0,cy:6,r:14,fill:'#f2c9a0',...OUT}))},
  // ice cream: cone behind the hand, scoop above it (the game hides the scoop when it drops)
  cone:{joint:'rHand',z:'back',draw:g=>{g.append(el('path',{d:'M-11,-4 L11,-4 L0,30 Z',fill:'#e0a458',...OUT}));g.append(el('path',{d:'M-7,6 L2,20 M0,0 L6,12',stroke:'#8a5a2b','stroke-width':1.5}));}},
  scoop:{joint:'rHand',z:'back',draw:g=>{g.append(el('circle',{cx:0,cy:-16,r:14,fill:'#f7b7cf',...OUT}));g.append(el('circle',{cx:-4,cy:-22,r:3,fill:'#fff',stroke:'none'}));}},
};
function buildRig(container){
  ...
  // squint '><' drawn over the closed lids (eyes=2)
  const squint=el('g',{class:'squint'});
  ...
  groups.eyewhites=[...groups.top.querySelectorAll('.eyewhite')];
  groups.props={};
  for(const name in PROPS){const p=PROPS[name],g=el('g',{class:'prop_'+name});g.style.display='none';p.draw(g);
    const host=groups[p.joint];
    if(p.z==='back')host.insertBefore(g,host.firstChild);
    else if(p.z==='mid')host.insertBefore(g,[...host.children].find(c=>/^j_/.test(c.getAttribute('class')||''))||null);
    else host.append(g);
    groups.props[name]=g;}
  return groups;
}
// Irises are clamped inside the eye whites (ellipse rx 9, ry 11 at (±12,-30), iris r 2.8) so they never poke out.
const EYE={ax:5.2,ay:7.2,cy:-30};
function clampIris(id,j,p){const cx=id==='lIris'?-12:12;let dx=j.px+p.x-cx,dy=j.py+p.y-EYE.cy;const k=Math.hypot(dx/EYE.ax,dy/EYE.ay);
  if(k<=1)return p;dx/=k;dy/=k;return {x:cx+dx-j.px,y:EYE.cy+dy-j.py,r:p.r};}
function applyPose(groups,pose){
  for(const id of ORDER){const j=R[id];let p=pose[id]||{x:0,y:0,r:0};
    ...
    if(id==='lIris'||id==='rIris')p=clampIris(id,j,p);
    groups[id].setAttribute('transform',`translate(${j.px+p.x},${j.py+p.y}) rotate(${p.r})`);}
  const marks=pose.eyes>=2;
  groups.lids.style.display=pose.eyes===1?'':'none';
  groups.squint.style.display=pose.eyes===2?'':'none';
  groups.xx.style.display=pose.eyes===3?'':'none';
  groups.eyewhites.forEach(e=>e.style.display=marks?'none':'');
  groups.lIris.style.display=groups.rIris.style.display=marks?'none':'';
}
function createDoll(container){const groups=buildRig(container);
  const d={groups,pose:defaultPose(),apply(){applyPose(groups,d.pose);},set(p){d.pose=clone(p);d.apply();},
    ...
    // arms in front of the head (hands over the eyes, ice cream at the mouth); default order draws them behind it
    setFront(f){for(const a of ['lUpper','rUpper'])f?groups.torso.append(groups[a]):groups.torso.insertBefore(groups[a],groups.head);}};
  d.apply();return d;}
...
const SFX=(()=>{
  ...
  const S={
    ...
    moo:()=>sweep('sawtooth',180,120,0.8,0.2),
    oink:()=>{sweep('square',500,250,0.12,0.2);sweep('square',520,240,0.12,0.2,0.16);},
    ...
    horn:()=>{sweep('square',330,330,0.3,0.15);sweep('square',392,392,0.35,0.15,0.3);},
    slurp:()=>sweep('sine',250,1400,0.18,0.2),
    snore:()=>{burst(0.5,'lowpass',300,0.5);sweep('sawtooth',70,55,0.5,0.15);sweep('sine',900,1300,0.4,0.08,0.7);},
    yawn:()=>sweep('sine',420,180,0.9,0.2),
  ...
  // A sound can have variants: fart.mp3, fart_1.mp3, fart_2.mp3 ... one is picked at random each play.
  function variants(n){const b=baked();const re=new RegExp('^'+n+'(_\\d+)?$');const v=Object.keys(b).filter(k=>re.test(k));return v.length?v:[n];}
  function play(n){if(!S[n])return;const vs=variants(n);const k=vs[Math.floor(Math.random()*vs.length)];
    if(files[k]===undefined&&baked()[k]){files[k]=new Audio(baked()[k]);}const f=files[k];
    ...
    a.addEventListener('canplaythrough',()=>{files[k]=a;a.play().catch(()=>{});},{once:true});
    a.addEventListener('error',()=>{files[k]=false;try{S[n]();}catch(e){}},{once:true});
    a.load();return a;}
  return {names:Object.keys(S),play,unlock:ac,setBase(b){base=b;for(const k in files)delete files[k];}};
})();

// ===== Player =====
// makePlayer(doll,{fx}): fx(name,frame,anim) is called when a keyframe with an `fx` field starts (the game draws overlays).
// anim.props (array of PROPS names) are shown for the duration of the animation; anim.front draws the arms in front of the head.
function makePlayer(doll,{fx}={}){
  let timer=null,onDone=null;
  function stop(){if(timer){clearInterval(timer);timer=null;}doll.setProps([]);doll.setFront&&doll.setFront(false);}
  function play(anim,{loop=false,step=true,done}={}){stop();if(!anim||!anim.frames.length){done&&done();return;}
    const fr=anim.frames,fps=anim.fps||8;let i=0,sub=0;onDone=done;doll.setProps(anim.props);doll.setFront&&doll.setFront(!!anim.front);
  ...
  const duration=a=>a.frames.reduce((s,k)=>s+k.hold,0)/(a.fps||8)*1000;
  return {play,stop,playing:()=>!!timer,duration};
}
// In-between frames: a keyframe held >= 2 ticks whose next pose differs a lot (a fall, a jump) spends its
// last tick on the halfway pose. Adds roughly 30% keyframes where motion is coarse, without changing timing.
function poseDelta(a,b){let m=0;for(const k in R){const p=a[k],q=b[k];m=Math.max(m,Math.abs(p.r-q.r),Math.abs(p.x-q.x)+Math.abs(p.y-q.y));}return m;}
function midFrame(a,b){const p=lerpPose(a.pose,b.pose,0.5);p.mouth=a.pose.mouth;p.eyes=a.pose.eyes;return {hold:1,sound:'',fx:'',pose:p};}
// A 1-tick keyframe followed by a huge change (the fall itself) gets a midway tick added instead (+125 ms).
function densify(anim,threshold=40,big=80){const fr=anim.frames,out=[];
  for(let i=0;i<fr.length;i++){const k=fr[i],n=fr[i+1];const d=n?poseDelta(k.pose,n.pose):0;
    if(k.hold>=2&&d>=threshold){out.push({...k,hold:k.hold-1});out.push(midFrame(k,n));}
    else if(k.hold===1&&d>=big){out.push(k);out.push(midFrame(k,n));}
    else out.push(k);}
  return {...anim,frames:out};}
...
function stopSpeech(){const f=VOICE.stopFn;VOICE.stopFn=null;if(f)f();}
function speak(text,doll,{lang='nl-NL',done}={}){
  ...
  const finish=()=>{if(ended)return;ended=true;VOICE.stopFn=null;clearInterval(mouthT);doll.pose.mouth=0;doll.apply();done&&done();};
  voiceUrl(text,lang).then(url=>{
    if(url){const a=new Audio(url);a.preservesPitch=false;a.mozPreservesPitch=false;a.playbackRate=VOICE.cfg.rate;a.onended=finish;a.onerror=finish;VOICE.stopFn=()=>{try{a.pause();}catch(e){}finish();};
    ...
    if(window.speechSynthesis){const u=new SpeechSynthesisUtterance(text);u.lang=lang;u.rate=0.95;u.pitch=1.8;u.onend=finish;u.onerror=finish;
      speechSynthesis.cancel();speechSynthesis.speak(u);startMouth();VOICE.stopFn=()=>{speechSynthesis.cancel();finish();};setTimeout(finish,chars.length*150+1500);}
...
function makeDefaultLib(){
  ...
  const flat=(dir)=>({torso:{r:88*dir},root:{y:40,x:dir>0?10:70}});   // falling left needs the hips further right to keep the head on screen
  const lib={
    ...
    slap_headspin:A(8,[F(4,{mouth:1,lIris:{x:3},rIris:{x:3}},'','ball'),
    ...
    // party: flags in both hands (arms in a V, flags upright), party hat, confetti; horn, hops, waving
    slap_party:(()=>{const wave=d=>({lUpper:{r:150+15*d},lLower:{r:0},lHand:{r:-150-5*d},rUpper:{r:-150-15*d},rLower:{r:0},rHand:{r:150+5*d}});   // hand r keeps the stick upright, minus a little tilt
      const hop=(i,d)=>({...wave(d),root:{y:i%2?-16:0},torso:{r:5*d},head:{r:-4*d},hat:{y:i%2?-6:0},mouth:i%2?4:1,eyes:0});
      return A(8,[F(2,{...wave(1),mouth:4,eyes:0},'horn','confetti'),F(2,{...wave(-1),mouth:4}),
        ...Array.from({length:12},(_,i)=>F(1,hop(i,i%4<2?1:-1),i%4===0?'pop':'')),
        F(2,{...wave(1),mouth:4,root:{y:-4}},'horn'),F(2,{...wave(-1),mouth:4}),F(2,{...wave(1),mouth:1,eyes:1}),F(2,base)],{props:['flagL','flagR','partyHat']});})(),
    // kiekeboe: hands over the eyes, a peek with one hand, big reveal, hide again, then the reveal with the hat jumping up
    slap_peekaboo:(()=>{const hide={lUpper:{r:-115},lLower:{r:-85},rUpper:{r:115},rLower:{r:85},mouth:0,eyes:1};
      const peek={...hide,lUpper:{r:-95},lLower:{r:-95},eyes:0,lIris:{x:-4},rIris:{x:-4}};
      const open={lUpper:{r:140},lLower:{r:0},lHand:{r:-30},rUpper:{r:-140},rLower:{r:0},rHand:{r:30},mouth:4,eyes:0};   // jazz hands
      return A(8,[F(3,hide),F(3,{...hide,torso:{r:4},head:{r:4}}),F(1,{...hide,torso:{r:-4},head:{r:-4}}),
        F(4,{...open,root:{y:-8}},'kiekeboe'),F(2,{...open,mouth:1}),
        F(3,{...hide,torso:{r:-4},head:{r:-4}}),F(2,peek),F(2,hide),F(1,peek),F(2,hide),
        F(4,{...open,root:{y:-10},hat:{y:-34,r:15}},'kiekeboe'),F(2,{...open,hat:{y:-12,r:5},mouth:1}),
        F(2,{...open,lUpper:{r:-40},rUpper:{r:40},lHand:{r:0},rHand:{r:0},hat:{y:0,r:0},mouth:1,eyes:1},'pop'),F(2,base)],{front:true,props:['bigHandL','bigHandR']});})(),
    // ice cream at the mouth (arm in front of the face): two licks, the scoop tips off onto his shoe, sad, shrug, eats the cone anyway
    slap_icecream:(()=>{const hold={rUpper:{r:75},rLower:{r:135},rHand:{r:150}};   // hand just below the mouth, cone upright
      return A(8,[F(3,{...hold,head:{r:6},mouth:0,lIris:{y:4},rIris:{y:4}}),F(2,{...hold,head:{r:10},mouth:1,lIris:{y:4},rIris:{y:4}},'slurp'),F(1,{...hold,head:{r:6},mouth:0,lIris:{y:4},rIris:{y:4}}),
        F(2,{...hold,head:{r:10},mouth:2,lIris:{y:4},rIris:{y:4}},'slurp'),F(3,{...hold,head:{r:-4},mouth:1,eyes:1},'mmm'),
        F(3,{...hold,rHand:{r:-165},head:{r:6},mouth:1,eyes:0,lIris:{x:3,y:5},rIris:{x:3,y:5}},'','scoopdrop'),
        F(2,{...hold,head:{r:16},mouth:4,eyes:0,lIris:{x:3,y:5},rIris:{x:3,y:5}}),
        F(4,{...hold,head:{r:12},torso:{r:4},mouth:1,eyes:1},'ohnee'),
        F(3,{...hold,lUpper:{r:-70},lLower:{r:-60},head:{r:-8},mouth:1,lIris:{x:-3},rIris:{x:3}}),
        F(2,{...hold,rUpper:{r:80},rLower:{r:140},head:{r:8},mouth:4}),F(2,{...hold,rUpper:{r:80},rLower:{r:140},head:{r:8},mouth:0},'pop'),F(2,{...hold,head:{r:0},mouth:1,eyes:1}),F(2,base)],{props:['cone','scoop'],front:true});})(),
    // trampoline: three hops, each higher; the hat stays hanging at the top after the last one and lands on his head
    slap_bounce:(()=>{const air={lUpper:{r:-40},rUpper:{r:40},lLower:{r:-30},rLower:{r:30}};
      return A(8,[F(2,{...crouch,mouth:1}),F(2,{root:{y:-25},...air,mouth:1},'boing'),F(1,{...crouch,mouth:1},'plof'),
        F(2,{root:{y:-45},...armsUp,mouth:4,hat:{y:-4}},'boing'),F(1,{...crouch,mouth:1},'plof'),
        F(2,{root:{y:-65},...armsUp,lThigh:{r:30},rThigh:{r:-30},mouth:4,eyes:2,hat:{y:-8}},'boing'),
        F(1,{root:{y:-30},...air,mouth:4,eyes:0,hat:{y:-43}}),                                      // hat stays where it was
        F(2,{...crouch,mouth:1,eyes:0,lIris:{y:-4},rIris:{y:-4},hat:{y:-79}},'plof'),F(2,{mouth:1,lIris:{y:-4},rIris:{y:-4},hat:{y:-73}}),
        F(1,{mouth:1,lIris:{y:-4},rIris:{y:-4},hat:{y:-40,r:-8}}),F(1,{mouth:1,eyes:0,hat:{y:-12,r:-4}}),
        F(1,{head:{y:4},mouth:4,eyes:2,hat:{y:0,r:0}},'bonk'),F(2,{head:{r:-10},mouth:1,eyes:2}),F(2,{head:{r:8},mouth:1,eyes:2}),F(2,base)]);})(),
    // dozes off: yawn, hat slides over the eyes, snores with Zs, a bird caws and he wakes up with a jump
    slap_sleep:(()=>{const inhale={head:{r:12},torso:{r:2,y:-2},eyes:1,mouth:1,hat:{y:22,r:6}},exhale={head:{r:16},torso:{r:6,y:1},eyes:1,mouth:0,hat:{y:24,r:8}};
      return A(8,[F(4,{...armsUp,mouth:4,eyes:1,torso:{r:-4},head:{r:-8}},'yawn'),F(2,{mouth:1,eyes:1,head:{r:6}}),F(2,{head:{r:10},torso:{r:4},eyes:1,hat:{y:12,r:4}}),
        F(5,inhale,'snore','zzz'),F(6,exhale,'','zzz'),F(5,inhale,'snore','zzz'),F(6,exhale,'','zzz'),
        F(2,exhale,'','bird'),
        F(2,{root:{y:-40},...armsUp,mouth:4,eyes:0,hat:{y:-30,r:-20},lThigh:{r:20},rThigh:{r:-20}},'boing'),F(1,{root:{y:-15},...armsUp,mouth:4,eyes:0,hat:{y:-10,r:-10}}),
        F(2,{...crouch,mouth:1,eyes:0,hat:{y:0}},'plof'),F(2,{head:{r:-15},mouth:1,lIris:{x:-5},rIris:{x:-5}}),F(2,{head:{r:15},mouth:1,lIris:{x:5},rIris:{x:5}}),F(2,base)]);})(),
  ...
  for(const n in lib)if(!n.startsWith('dance_'))lib[n]=densify(lib[n]);
  return lib;
}
